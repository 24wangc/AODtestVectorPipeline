...
struct JetCollection {
    ...
    std::unordered_map<std::string, std::vector<double>> leading_double_vars;
    std::unordered_map<std::string, std::vector<double>> subleading_double_vars;
...
template<typename Container, typename Jet, typename GetEt>
void fillJetCollectionOneEvent(const Container* jets, JetCollection& jc, GetEt getEt) {
    if(!jets) return;
...
void HDF5er(bool vbfBool, bool signalBool, int jzSlice) {
    ...
    h5.createGroup("/CaloTopoTowers");
    h5.createGroup("/CaloTopoTowers/Et");

    h5.createGroup("/topo422");
    h5.createGroup("/topo422/Et");

    h5.createGroup("/InTimeAntiKt4TruthJets");
    h5.createGroup("/InTimeAntiKt4TruthJets/Et");

    h5.createGroup("/AntiKt4TruthDressedWZJets");
    h5.createGroup("/AntiKt4TruthDressedWZJets/Et");

    h5.createGroup("/gFEXSRJ");
    h5.createGroup("/gFEXSRJ/Et");

    h5.createGroup("/gFEXLRJ");
    h5.createGroup("/gFEXLRJ/Et");

    h5.createGroup("/jFEXSRJ");
    h5.createGroup("/jFEXSRJ/Et");

    h5.createGroup("/jFEXLRJ");
    h5.createGroup("/jFEXLRJ/Et");

    h5.createGroup("/HLTJets");
    h5.createGroup("/HLTJets/Et");

    h5.createGroup("/RecoAK10");
    h5.createGroup("/RecoAK10/Et");
    ...
    write_vector(h5, "/EventInfo/sumOfSeightsForSample", sumOfWeightsVec);
